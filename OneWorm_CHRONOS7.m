...
Alldata = DataDir
    ...
    mkdir([Alldata filesep TrialName 'RUNfinal']);
    OutDir2=[TrialName 'RUNfinal'];      
    %OutDir = [Alldata '/RESULTS']
    %a = [Alldata '/'];
...
for W=1:length(DateFldrNms)
    ...
    dirOutputtif = dir(fullfile([Alldata, '/',DateFldrNms{W}], '*tif')); %list the images
    if size(dirOutputtif,1) < 1
        dirOutputtif = dir(fullfile([Alldata, '/',DateFldrNms{W}], '*TIF')); %list the images
    end
    
    cd([Alldata, '/',DateFldrNms{W}]);
    imgtmp=imread(dirOutputtif(1).name); %load the first image in the folder
...
for W=FldStart:FldMax; %loop Well folders
    ...
    dirOutput2 = dir(fullfile([Alldata, '/',DateFldrNms{W}], '*tif*')); %list the images
...
'(majAx/minAx)/extent'}
    ...
    % date indicated 
      cd(Alldata);
          ...
          save ([OutDir2, filesep, 'Params'], 'Params')
          save ([OutDir2 filesep DateFldrNms{W},'_',DateFldrNms2{ImN}, 'final.mat'],  'Imagesfilt', 'ImgPropHeaders', 'F', 'ProcessDate',...
          'filtVal', 'FltNm2' ,'imgBWL', 'img1', 'Img_Propfilt', 'Image_PropertiesAll');
